...
The RPM spec file provided here is for INTICOIN-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://INTICOIN.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[INTICOIN git repository](https://github.com/INTICOIN/INTICOIN) but are not part of
...
Currently only `Source10` is used, for the example `INTICOIN.conf` file.
...
in the INTICOIN git repository. At present only an SVG version of the INTICOIN
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the INTICOIN
...
needed when 0.12.1 is released, a proper fix is already in the INTICOIN git
master branch.
...
idea to build INTICOIN Core against a static version of that library compiled
...
The `INTICOIN.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `INTICOIN-server` package. At this point, most RPM based
...
The `INTICOIN-server` package should have SELinux support. How to properly do
...
INTICOIN-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/INTICOIN/). He has been
packaging INTICOIN for Fedora at least since 2012.
...
4. I renamed the `INTICOIN` package that contains the Qt GUI to `INTICOIN-core` as
that appears to be how the general population refers to it, in contrast to
`INTICOIN-xt` or `INTICOIN-classic`. I wanted to make sure the general population
