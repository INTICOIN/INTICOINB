...
###   This script attempts to download the signature file SHA256SUMS.asc from INTICOIN.org
...
WORKINGDIR="/tmp/INTICOIN"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://INTICOIN.org/bin/"
VERSIONPREFIX="INTICOIN-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'INTICOIN-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the INTICOIN Core binary release signing key installed?"
   fi
