        ...
        if(pblocktree->GetPaymentRequestIndex(vec))
        {
            BOOST_FOREACH(const CFund::CPaymentRequest& prequest, vec) {
                ...
                if(prequest.fState == CFund::ACCEPTED && prequest.paymenthash == uint256() &&
                    ...
                    CFund::CProposal proposal;
                    if(CFund::FindProposal(prequest.proposalhash, proposal)) {
                        CINTICOINAddress addr(proposal.Address);
                        if (!addr.IsValid())
                            continue;
                        coinbaseTx.vout.resize(coinbaseTx.vout.size()+1);
...
void INTICOINStaker(const CChainParams& chainparams)
{
    LogPrintf("INTICOINStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("INTICOIN-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in INTICOINStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running INTICOINStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("INTICOINStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("INTICOINStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("INTICOINStaker: ProcessNewBlock, block not accepted");
        }
        else
        {
