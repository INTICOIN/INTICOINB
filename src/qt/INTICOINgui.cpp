...
#include "config/INTICOIN-config.h"
#endif

#include "main.h"
#include "INTICOINgui.h"
#include "INTICOINunits.h"
...
const std::string INTICOINGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString INTICOINGUI::DEFAULT_WALLET = "~Default";

INTICOINGUI::INTICOINGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void INTICOINGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/INTICOIN"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a INTICOIN address"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your INTICOIN addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified INTICOIN addresses"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible INTICOIN command-line options").arg(tr(PACKAGE_NAME)));
...
void INTICOINGUI::bootstrapBlockchain()
{
    bool ok = false;
    QString defaultUrl = "https://nav.nyc3.digitaloceanspaces.com/bootstrap/mainnet/bootstrap-INTICOIN_" +
            QString::fromStdString(Params().NetworkIDString()) + "net.tar";
    QString url = QInputDialog::getText(this, tr("Bootstrap blockchain"),
                                            tr("You can use an external trusted source to download the blockchain from.<BR>The following URL points to a bootstrap copy provided by the INTICOIN Core Team.<BR>Where would you like to download it from?"), QLineEdit::Normal,
                                            defaultUrl, &ok);
    if (ok && !url.isEmpty())
    {
...
void INTICOINGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(INTICOINUnits::Unit u, INTICOINUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(INTICOINUnits::name(u)), this);
...
void INTICOINGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void INTICOINGUI::createToolBars()
{
    if(walletFrame)
    {
...
void INTICOINGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool INTICOINGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void INTICOINGUI::startVotingCounter()
{
    if (GetStaking())
    {
...
bool INTICOINGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void INTICOINGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void INTICOINGUI::setWalletActionsEnabled(bool enabled)
{
...
void INTICOINGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void INTICOINGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void INTICOINGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void INTICOINGUI::optionsClicked()
{
...
void INTICOINGUI::showDebugWindow()
{
...
void INTICOINGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void INTICOINGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void INTICOINGUI::openClicked()
{
...
void INTICOINGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void INTICOINGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void INTICOINGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void INTICOINGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void INTICOINGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void INTICOINGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void INTICOINGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void INTICOINGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
...
void INTICOINGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to INTICOIN network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to INTICOIN network..."));
...
void INTICOINGUI::showVotingDialog()
{
...
void INTICOINGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("INTICOIN"); // default title
...
void INTICOINGUI::changeEvent(QEvent *e)
{
...
void INTICOINGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void INTICOINGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void INTICOINGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(INTICOINUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void INTICOINGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void INTICOINGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool INTICOINGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool INTICOINGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void INTICOINGUI::setEncryptionStatus(int status)
{
...
void INTICOINGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void INTICOINGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void INTICOINGUI::detectShutdown()
{
...
void INTICOINGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void INTICOINGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(INTICOINGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void INTICOINGUI::subscribeToCoreSignals()
{
...
void INTICOINGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<INTICOINUnits::Unit> units = INTICOINUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const INTICOINUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(INTICOINUnits::name(unit)));
    }
...
{
    setText(INTICOINUnits::name(newUnits));
}
...
void INTICOINGUI::toggleStaking()
{
...
void INTICOINGUI::replyFinished(QNetworkReply *reply)
{
...
void INTICOINGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CINTICOINAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void INTICOINGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.INTICOIN.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void INTICOINGUI::updateStakingStatus()
{
    updateWeight();
