...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total INTICOIN balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CINTICOINAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given INTICOIN address.\n"
            "\nArguments:\n"
            "1. \"INTICOINaddress\"     (string, required) The INTICOIN address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"INTICOINaddress\", (string) The INTICOIN address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: INTICOIN address and we have full public key:
        CINTICOINAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are INTICOIN addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) INTICOIN address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CINTICOINAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CINTICOINAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"INTICOINaddress\"  (string, required) The INTICOIN address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CINTICOINAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CINTICOINAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CINTICOINAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
