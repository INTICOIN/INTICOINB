...
const char * const INTICOIN_CONF_FILENAME = "INTICOIN.conf";
const char * const INTICOIN_PID_FILENAME = "INTICOIN.pid";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "INTICOIN4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/INTICOIN4";
#else
    // Unix
    return pathRet / ".INTICOIN4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", INTICOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No INTICOIN.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override INTICOIN.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", INTICOIN_PID_FILENAME));
